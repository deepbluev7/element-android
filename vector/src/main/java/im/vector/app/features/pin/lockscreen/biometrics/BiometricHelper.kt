...
class BiometricHelper @Inject constructor(
    ...
    private fun authenticateWithPromptInternal(
        activity: FragmentActivity,
        cryptoObject: BiometricPrompt.CryptoObject? = null,
    ...
    private fun getAvailableAuthenticators(): Int {
        ...
        if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.Q) {
            if (authenticators == 0 && canUseDeviceCredentialsAuth) {
                authenticators = DEVICE_CREDENTIAL
            }
            if (authenticators == 0 && canUseStrongBiometricAuth) {
                authenticators = BIOMETRIC_STRONG
            }
            if (authenticators == 0 && canUseWeakBiometricAuth) {

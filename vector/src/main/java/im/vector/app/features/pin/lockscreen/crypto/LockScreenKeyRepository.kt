...
class LockScreenKeyRepository(
    ...
    private val systemKeyHelper = SystemKeyHelper.create(systemKeyAlias)

    /**
     * Encrypts the [pinCode], creating the associated key if needed.
     */
    fun encryptPinCode(pinCode: String): String {
        return with(ByteArrayOutputStream()) {
            secureStorageService.securelyStoreObject(pinCode, pinCodeKeyAlias, this)
     ...
     */
    fun decryptPinCode(encodedPinCode: String): String? {
        val bytes = Base64.decode(encodedPinCode, Base64.NO_WRAP)
        return secureStorageService.loadSecureSecret(bytes.inputStream(), pinCodeKeyAlias)
    }
    ...
    fun ensureSystemKey() {
        val key = systemKeyHelper.ensureKey()
        systemKeyHelper.initializeKeyOrThrow(key)
    }

    /**
     * Returns if the PIN code key already exists.
     */
    fun hasPinCodeKey() = keyStore.containsAlias(pinCodeKeyAlias)

    /**
     * Returns if the system authentication key already exists.
     */
    fun hasSystemKey() = keyStore.containsAlias(systemKeyAlias)

    /**
     * Deletes the PIN code key from the KeyStore.
     */
    fun deletePinCodeKey() = keyStore.deleteEntry(pinCodeKeyAlias)

    /**
     * Deletes the system authentication key from the KeyStore.
     */
    fun deleteSystemKey() = keyStore.deleteEntry(systemKeyAlias)

    /**
     * Checks if the current system authentication key exists and is valid.
     */
    fun isSystemKeyValid() = hasSystemKey() && systemKeyHelper.hasValidKey()
